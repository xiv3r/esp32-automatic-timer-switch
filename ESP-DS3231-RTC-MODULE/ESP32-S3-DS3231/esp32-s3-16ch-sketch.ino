...
static const unsigned long DS3231_READ_INTERVAL =      1000UL;
...
bool readDS3231Time(time_t& epoch) {
    if (!ds3231Present) return false;
    ...
    return (epoch > 1577836800UL && epoch < 4102444800UL);
}

bool writeDS3231Time(time_t epoch) {
    if (!ds3231Present) return false;
    ...
    Wire.beginTransmission(DS3231_ADDR);
    Wire.write(DS3231_REG_SEC);
    Wire.write(decToBcd(ti->tm_sec));
    Wire.write(decToBcd(ti->tm_min));
    Wire.write(decToBcd(ti->tm_hour));
    Wire.write(decToBcd(ti->tm_wday + 1));
    Wire.write(decToBcd(ti->tm_mday));
    Wire.write(decToBcd(ti->tm_mon + 1));
    Wire.write(decToBcd(ti->tm_year % 100));
...
function tick(){fetch('/api/time').then(r=>r.json()).then(d=>{document.getElementById('clk').textContent=d.time||'--:--:--';const w=document.querySelector('.wd'),t=document.querySelector('.td');if(w)w.className='dot '+(d.wifi?'g':'r');if(t){let tc='y';if(d.timeSource==='ntp')tc='g';else if(d.timeSource==='ds3231')tc='b';else if(d.timeSource==='browser')tc='b';else tc='y';t.className='dot '+tc;}}).catch(()=>{});}
...
<div id="ds3231Status" class="alert" style="display:none;margin-bottom:12px"></div>
<div class="fg">
<label>NTP Server</label>
<input type="text" id="srv" placeholder="pool.ntp.org" required>
<small>Fallbacks: pool.ntp.org &rarr; time.nist.gov &rarr; ntp1.time.nl &rarr; nts.netnod.se</small>
</div>
...
<small style="display:block;margin-top:12px;color:#90A4AE">Use <strong>Sync Browser</strong> when NTP servers are unreachable. NTP will automatically override browser time when available.</small>
</div>
</main>
<div id="toast"></div>
<script>
...
function updateTimeStatus(d){
  ...
  if(d.timeSource==='ntp'){
    ...
    cls='ai';
  }else if(d.timeSource==='ds3231'){
    icon='&#x1F4C5;';msg='Time source: <strong>DS3231 Hardware RTC</strong> — Excellent accuracy, battery backed';
    cls='ai';
...
function checkDS3231(){
  fetch('/api/ds3231').then(r=>r.json()).then(d=>{
    const statusDiv=document.getElementById('ds3231Status');
    statusDiv.style.display='block';
    if(d.present){
      statusDiv.innerHTML='✅ <strong>DS3231 RTC detected</strong> — Hardware clock active on GPIO16(SDA)/GPIO17(SCL)';
      statusDiv.className='alert ai';
    }else{
      statusDiv.innerHTML='⚠️ <strong>DS3231 RTC not detected</strong> — Using internal RTC (less accurate during power loss). Connect DS3231 to GPIO16(SDA) and GPIO17(SCL) for better timekeeping.';
      statusDiv.className='alert aw';
    }
  }).catch(()=>{});
}
setInterval(tick,1000);tick();
setInterval(checkDS3231,30000);
checkDS3231();
...
<strong>ℹ️ Reserved Pins:</strong> GPIO16 (SDA) and GPIO17 (SCL) are reserved for DS3231 RTC <span id="ds3231Badge"></span>. These pins are automatically excluded from relay configuration.
</div>
<div class="fg">
...
function updateDS3231Badge() {
    fetch('/api/ds3231').then(r=>r.json()).then(d=>{
        const badge = document.getElementById('ds3231Badge');
        if(d.present){
            badge.innerHTML = '<span style="color:#2E7D32">(detected and active)</span>';
        }else{
            badge.innerHTML = '<span style="color:#F9A825">(not detected - RTC optional)</span>';
        }
    }).catch(()=>{});
}
updateDS3231Badge();
setInterval(updateDS3231Badge,30000);
...
function loadGPIO() {
    ...
    if(data.mode !== undefined) {
        ...
        const reservedPins = new Set([16,17]);
        d.availablePins.forEach(pin => {
            if(!usedPins.has(pin) && !reservedPins.has(pin)) {
...
<div class="ibox"><div class="l">DS3231 RTC</div><div class="v" id="sds3231" style="font-size:13px">&hellip;</div></div>
</div>
<div class="card fcrd">
...
<p style="color:#90A4AE;font-size:12px;margin-top:10px">Factory reset clears all settings including WiFi credentials, schedules, AP configuration, and GPIO pin assignments.<br>Alternatively, hold the <strong>BOOT button</strong> (GPIO 0) for 5 seconds during normal operation to trigger a hardware factory reset.</p>
</div>
</main>
<div id="toast"></div>
<script>
...
function loadSys(){
  fetch('/api/system').then(r=>r.json()).then(d=>{
    ...
    else if(tsrc==='DS3231')tsrcStyle='color:#1565C0';
    else if(tsrc==='Browser')tsrcStyle='color:#FB8C00';
    ...
    const ds3231El = document.getElementById('sds3231');
    if(ds3231El) {
        if(d.ds3231Present) {
            ds3231El.innerHTML = '<span style="color:#2E7D32">✓ Connected (GPIO16/17)</span>';
            ds3231El.title = 'DS3231 hardware RTC active - Accurate time keeping without WiFi';
        } else {
            ds3231El.innerHTML = '<span style="color:#C62828">✗ Not detected</span>';
            ds3231El.title = 'DS3231 not found - Using internal RTC (optional add-on)';
        }
    }
  }).catch(()=>{});
}
...
time_t getCurrentEpoch() {
    if (ds3231Present && (sysConfig.use_ds3231 || timeSource == TIME_SOURCE_NONE)) {
        time_t ds3231Time = 0;
        if (readDS3231Time(ds3231Time) && ds3231Time > 1577836800UL) {
            if (internalEpoch != ds3231Time) {
                internalEpoch = ds3231Time;
                internalMillisAtLastSync = millis();
                rtcMicrosAtLastSync = micros();
                lastRTCRebase = millis();
                rtcInitialized = true;
                if (timeSource == TIME_SOURCE_NONE) {
                    timeSource = TIME_SOURCE_DS3231;
                }
            }
            return ds3231Time;
        }
...
void saveRTCState() {
    ...
    if (ds3231Present && internalEpoch > 1577836800UL) {
        writeDS3231Time(internalEpoch);
    }
}
...
void loadRTCState() {
    ...
    if (ds3231Present) {
        ...
        if (readDS3231Time(ds3231Time) && ds3231Time > 1577836800UL) {
            ...
            lastRTCRebase = millis();
            rtcInitialized = true;
            if (timeSource == TIME_SOURCE_NONE || timeSource == TIME_SOURCE_BROWSER) {
                timeSource = TIME_SOURCE_DS3231;
            }
        }
    }
...
void handleBrowserTimeSync() {
    ...
    if (ds3231Present) {
        writeDS3231Time(browserUtcEpoch);
    }
...
void startMDNS() {
    ...
    if (MDNS.begin(mdnsHostname)) {
        ...
        MDNS.addServiceTxt("http", "tcp", "version", "v8");
        MDNS.addServiceTxt("http", "tcp", "channels", String(gpioConfig.count).c_str());
        if (ds3231Present) {
            MDNS.addServiceTxt("http", "tcp", "rtc", "DS3231");
        }
        mdnsStarted = true;
    } else {
        mdnsStarted = false;
...
void loop() {
    ...
    if (ds3231Present && timeHasElapsed(now, lastDS3231Read, DS3231_READ_INTERVAL)) {
        lastDS3231Read = now;
        time_t ds3231Time;
        if (readDS3231Time(ds3231Time) && ds3231Time > 1577836800UL) {
            if (abs(ds3231Time - internalEpoch) > 2) {
                internalEpoch = ds3231Time;
                internalMillisAtLastSync = now;
                rtcMicrosAtLastSync = micros();
                if (timeSource == TIME_SOURCE_NONE || timeSource == TIME_SOURCE_BROWSER) {
                    timeSource = TIME_SOURCE_DS3231;
                }
...
void setupWebServer() {
    ...
    server.on("/api/ds3231", HTTP_GET, []() {
        String resp = "{\"present\":" + String(ds3231Present ? "true" : "false") + 
                      ",\"sda\":" + String(DS3231_SDA_PIN) + 
                      ",\"scl\":" + String(DS3231_SCL_PIN) + "}";
        server.send(200, "application/json", resp);
    });
...
void handleGetTime() {
    ...
    String sourceStr;
    if (timeSource == TIME_SOURCE_NTP) sourceStr = "ntp";
    else if (timeSource == TIME_SOURCE_BROWSER) sourceStr = "browser";
    else if (timeSource == TIME_SOURCE_DS3231) sourceStr = "ds3231";
    else sourceStr = "none";
    
    String resp = "{\"time\":\"" + ts + "\",\"wifi\":" + 
                  String(wifiConnected ? "true" : "false") + ",\"ntp\":" + 
                  String((timeSource == TIME_SOURCE_NTP) ? "true" : "false") + 
                  ",\"timeSource\":\"" + sourceStr + "\",\"ds3231\":" + 
                  String(ds3231Present ? "true" : "false") + "}";
    server.send(200, "application/json", resp);
}
...
void handleGetNTP() {
    String resp = "{\"ntpServer\":\"" + String(sysConfig.ntp_server) + 
                  ...
                  ",\"globalActiveMode\":" + String(extConfig.global_active_mode) +
                  ",\"useDS3231\":" + String(sysConfig.use_ds3231 ? "true" : "false") +
                  ",\"ds3231Present\":" + String(ds3231Present ? "true" : "false") + "}";
    server.send(200, "application/json", resp);
}
...
void handleSaveNTP() {
    ...
    if (srv && strlen(srv) > 0 && strlen(srv) < 48) {
        ...
        if (doc.containsKey("useDS3231")) {
            sysConfig.use_ds3231 = doc["useDS3231"];
        }
...
void handleGetSystem() {
    ...
    doc["ds3231Present"] = ds3231Present;
    doc["ds3231Enabled"] = sysConfig.use_ds3231;
