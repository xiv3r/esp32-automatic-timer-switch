/*
============================================================
 *  ESP32-S3 24-Channel Relay Smart Switch — Firmware v5.2
 *  Added Day-of-Week + Day-of-Month Scheduling Support
 *  RTC: DS3231 on I2C (GPIO8=SDA, GPIO9=SCL)
 *  GPIO: 4,5,6,7,15,16,17,18,10,11,12,13,14,1,2,42,41,40,39,38,45,48,47,21
============================================================
 */
...
// ─── mDNS settings ────────────────────────────────────────────────────────────
#define MDNS_HOSTNAME_DEFAULT "esp32-s3-24ch-relay"
...
const int  relayPins[NUM_RELAYS] = { 
    4, 5, 6, 7, 15, 16, 17, 18, 
    10, 11, 12, 13, 14, 1, 2, 42, 
    41, 40, 39, 38, 45, 48, 47, 21
};
...
// AP copies
char ap_ssid[32]     = "ESP32_S3_24CH_Timer_Switch";
...
<header>
<span class="logo">&#x26A1; ESP32-S3 24-CH</span>
<nav>
...
void startMDNS() {
    ...
    while (retries-- > 0) {
        if (MDNS.begin(mdnsHostname)) {
            ...
            MDNS.addServiceTxt("http", "tcp", "model", "ESP32-S3-24CH-Relay");
            MDNS.addServiceTxt("http", "tcp", "version", "v5.2");
            MDNS.addServiceTxt("http", "tcp", "channels", "24");
...
void initDefaults() {
    ...
    strcpy(sysConfig.ap_ssid,     "ESP32_S3_24CH_Relay");
