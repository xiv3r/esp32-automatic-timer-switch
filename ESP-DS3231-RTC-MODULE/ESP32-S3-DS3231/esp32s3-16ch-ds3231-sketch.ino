...
// ─── mDNS settings ────────────────────────────────────────────────────────────
#define MDNS_HOSTNAME_DEFAULT "esp32-s3-16ch-relay"
...
.g{background:#69F0AE}.r{background:#FF5252}.y{background:#FFD740}.b{background:#42A5F5}.p{background:#9C27B0}
...
<title>Relays — ESP32-S3 16-CH</title>
<link rel="stylesheet" href="/style.css"></head><body>
<header>
<span class="logo">&#x26A1; ESP32-S3 16-CH</span>
<nav>
...
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>WiFi — ESP32-S3 16-CH</title>
<link rel="stylesheet" href="/style.css"></head><body>
<header>
<span class="logo">&#x26A1; ESP32-S3 16-CH</span>
<nav>
...
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Time — ESP32-S3 16-CH</title>
<link rel="stylesheet" href="/style.css"></head><body>
<header>
<span class="logo">&#x26A1; ESP32-S3 16-CH</span>
<nav>
...
<small style="display:block;margin-top:12px;color:#90A4AE">Use <strong>Sync from Browser</strong> when NTP servers are unreachable. Both browser and NTP provide UTC time, which is converted using your GMT offset. NTP will automatically override browser time when available. DS3231 RTC provides battery-backed time if installed.</small>
</div>
</main>
<div id="toast"></div>
<script>
...
function updateTimeStatus(d){
  ...
  if(d.timeSource==='ntp'){
    ...
    icon='&#x1F4C5;';msg='Time source: <strong>DS3231 RTC</strong> — Battery-backed hardware clock';
    cls='ai';
...
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>GPIO — ESP32-S3 16-CH</title>
<link rel="stylesheet" href="/style.css"></head><body>
<header>
<span class="logo">&#x26A1; ESP32-S3 16-CH</span>
<nav>
...
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>System — ESP32-S3 16-CH</title>
<link rel="stylesheet" href="/style.css"></head><body>
<header>
<span class="logo">&#x26A1; ESP32-S3 16-CH</span>
<nav>
...
<div class="ibox" id="dsbox" style="display:none"><div class="l">DS3231 Temp</div><div class="v" id="stmp">&hellip;</div></div>
</div>
...
function loadSys(){
  fetch('/api/system').then(r=>r.json()).then(d=>{
    ...
    else if(tsrc==='DS3231')tsrcStyle='color:#9C27B0';
    ...
    if(d.ds3231Present){
      document.getElementById('dsbox').style.display='';
      document.getElementById('stmp').textContent=d.ds3231Temp+'°C';
    }
  }).catch(()=>{});
}
...
void initDefaults() {
    ...
    strcpy(sysConfig.hostname, "esp32s3relay");
...
void loadConfiguration() {
    ...
    if (configValid && sysConfig.version < 6) {
        sysConfig.version = EEPROM_VERSION;
        saveConfiguration();
    }
...
void handleGetTime() {
    ...
    String timeSrc = "none";
    if (timeSource == TIME_SOURCE_NTP) timeSrc = "ntp";
    else if (timeSource == TIME_SOURCE_BROWSER) timeSrc = "browser";
    else if (timeSource == TIME_SOURCE_DS3231) timeSrc = "ds3231";
...
void handleGetSystem() {
    ...
    doc["timeSource"] = String(timeSource == TIME_SOURCE_NTP ? "NTP" : 
                               timeSource == TIME_SOURCE_DS3231 ? "DS3231" :
    ...
    if (ds3231Present) {
        doc["ds3231Temp"] = rtc.getTemperature();
    }
...
void handleGetGPIOConfig() {
    ...
    int validPins[] = {0, 1, 2, 3, 4, 5, 6, 7, 10, 11, 12, 13, 14, 15, 16, 17, 18};
    for (int p : validPins) {
        available.add(p);
    }
