...
.g{background:#69F0AE}.r{background:#FF5252}.y{background:#FFD740}.b{background:#42A5F5}.p{background:#CE93D8}
...
.as{background:#E8F5E9;border-left:4px solid #43A047;color:#1B5E20}
...
<title>Relays — ESP32 16-CH</title>
<link rel="stylesheet" href="/style.css"></head><body>
<header>
<span class="logo">&#x26A1; ESP32 16-CH</span>
<nav>
...
function tick(){fetch('/api/time').then(r=>r.json()).then(d=>{document.getElementById('clk').textContent=d.time||'--:--:--';const w=document.querySelector('.wd'),t=document.querySelector('.td');if(w)w.className='dot '+(d.wifi?'g':'r');if(t){let tc='y';if(d.timeSource==='ntp')tc='g';else if(d.timeSource==='browser')tc='b';else if(d.timeSource==='ds3231')tc='p';else tc='y';t.className='dot '+tc;}}).catch(()=>{});}
...
<div id="rtcStatus" class="alert as" style="display:none;margin-bottom:12px"></div>
<div class="fg">
...
<small style="display:block;margin-top:12px;color:#90A4AE">Use <strong>Sync from Browser</strong> when NTP servers are unreachable. Both browser and NTP provide UTC time, which is converted using your GMT offset. NTP will automatically override browser time when available. DS3231 RTC provides battery-backed timekeeping.</small>
</div>
</main>
<div id="toast"></div>
<script>
...
function updateTimeStatus(d){
  ...
  const r=document.getElementById('rtcStatus');
  ...
  if(d.timeSource==='ntp'){
    ...
    icon='&#x1F4C5;';msg='Time source: <strong>DS3231 RTC</strong> — Battery-backed accuracy';
    cls='as';
  }else{
  ...
  if(d.hasOwnProperty('rtcAvailable')){
    r.style.display='block';
    if(d.rtcAvailable){
      r.innerHTML='&#x1F4C5; <strong>DS3231 RTC Detected</strong> — Temperature: '+d.rtcTemp.toFixed(1)+'°C';
      r.className='alert as';
    }else{
      r.innerHTML='&#x26A0;&#xFE0F; <strong>DS3231 Not Detected</strong> — Check connections';
      r.className='alert aw';
    }
  }
}
setInterval(tick,1000);tick();
...
<div class="card fcrd">
<div class="alert ai">Configure which GPIO pins control your relays and set active level. Changes take effect immediately. Maximum 16 relays.</div>
...
<small>Only available GPIOs are shown. Pins used for flash/PSRAM and already configured pins are excluded.</small>
</div>
...
<button class="btn bwarn" onclick="resetDefaults()" style="padding:9px 18px">&#x1F504; Reset to Default 16 Pins</button>
</div>
</div>
</main>
<div id="toast"></div>
...
const DEFAULT_PINS = [15,2,4,5,18,3,1,23,13,12,14,27,26,25,33,32];
let gpioData = null;
...
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>System — ESP32 16-CH</title>
<link rel="stylesheet" href="/style.css"></head><body>
<header>
<span class="logo">&#x26A1; ESP32 16-CH</span>
<nav>
...
<div class="ibox"><div class="l">RTC Temp</div><div class="v" id="srtct">&hellip;</div></div>
...
function loadSys(){
  fetch('/api/system').then(r=>r.json()).then(d=>{
    ...
    else if(tsrc==='DS3231')tsrcStyle='color:#7B1FA2';
    ...
    document.getElementById('srtc').textContent=d.rtcAvailable ? (d.rtcTimeValid ? 'Active' : 'Not Set') : 'Not Detected';
    document.getElementById('srtc').style.color=d.rtcAvailable ? '#2E7D32' : '#C62828';
    document.getElementById('srtct').textContent=d.rtcTemp ? d.rtcTemp.toFixed(1)+'°C' : '\u2014';
...
// =============================================================================
//  BOOT BUTTON FACTORY RESET DETECTION
// =============================================================================
...
void loadGPIOConfig() {
    ...
    if (len != sizeof(GPIOPinConfig) || gpioConfig.magic != GPIO_CONFIG_MAGIC || 
        ...
        if (len == sizeof(GPIOPinConfig) - (MAX_RELAYS * sizeof(bool)) && 
            gpioConfig.magic == 0xD001 && gpioConfig.count > 0 && gpioConfig.count <= MAX_RELAYS) {
...
time_t getCurrentEpoch() {
    ...
    if (rtcAvailable && rtcTimeValid) {
        DateTime now = rtc.now();
        time_t ds3231Epoch = now.unixtime();
        if (ds3231Epoch > 1000000000UL && ds3231Epoch < 2000000000UL) {
...
void syncInternalRTC(time_t rawUtcEpoch) {
    ...
    if (rtcAvailable) {
        rtc.adjust(DateTime(rawUtcEpoch));
        rtcTimeValid = true;
    }
...
void handleBrowserTimeSync() {
    ...
    if (rtcAvailable) {
        rtc.adjust(DateTime(browserUtcEpoch));
        rtcTimeValid = true;
    }
...
void processRelaySchedules() {
    ...
    for (int i = 0; i < gpioConfig.count; i++) {
        if (relayConfigs[i].manualOverride) {
            ...
            if (!relayOutputsInitialized || lastRelayOutputs[i] != targetState) {
        ...
        if (!scheduleActiveCache[i]) {
            if (!relayOutputsInitialized || lastRelayOutputs[i] != false) {
        ...
        if (!relayOutputsInitialized || lastRelayOutputs[i] != on) {
            setRelayOutput(i, on);
...
void handleGetTime() {
    ...
    else if (timeSource == TIME_SOURCE_DS3231) timeSourceStr = "ds3231";
    ...
    String resp = "{\"time\":\"" + ts + "\",\"wifi\":" + 
                  ...
                  ",\"rtcAvailable\":" + String(rtcAvailable ? "true" : "false") +
...
void handleGetNTP() {
    String resp = "{\"ntpServer\":\"" + String(sysConfig.ntp_server) + 
                  ...
                  ",\"rtcAvailable\":" + String(rtcAvailable ? "true" : "false") +
                  ",\"rtcTimeValid\":" + String(rtcTimeValid ? "true" : "false") +
                  ",\"timeSource\":\"" + String(timeSource == TIME_SOURCE_NTP ? "ntp" : 
                                                timeSource == TIME_SOURCE_BROWSER ? "browser" : 
                                                timeSource == TIME_SOURCE_DS3231 ? "ds3231" : "none") + "\"}";
    server.send(200, "application/json", resp);
}
...
void handleGetSystem() {
    ...
    if (rtcAvailable) {
        doc["rtcTemp"] = rtc.getTemperature();
    } else {
        doc["rtcTemp"] = 0;
    }
...
void handleGetGPIOConfig() {
    ...
    int validPins[] = {0, 2, 3, 4, 5, 12, 13, 14, 15, 18, 23, 25, 26, 27, 32, 33};
    for (int p : validPins) {
        available.add(p);
    }
