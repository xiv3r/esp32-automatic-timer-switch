/*
============================================================
 *  ESP32 16-Channel Relay Smart Switch (Production-Grade Version)
 *  Author: Raff Alds
...
void initRTC() {
    Wire.begin(21, 22); 
    if (!rtc.begin()) {
        Serial.println("DS3231 RTC not found - continuing without hardware RTC");
        rtcPresent = false;
        return;
    }
    
    rtcPresent = true;
    Serial.println("DS3231 RTC detected on GPIO21(SDA)/GPIO22(SCL)");
    
    if (rtc.lostPower()) {
        Serial.println("DS3231 lost power - will sync from NTP/browser when available");
        rtcTimeValid = false;
    } else {
        ...
        if (now.year() >= 2020 && now.year() <= 2100) {
            ...
            if (rtcEpoch > 1000000000UL && rtcEpoch < 2000000000UL) {
                ...
                Serial.println("DS3231 RTC time loaded successfully");
...
void syncDS3231FromInternalRTC() {
    ...
    rtc.adjust(dt);
    rtcTimeValid = true;
    Serial.println("DS3231 RTC updated from internal time");
}
...
<p style="color:#90A4AE;font-size:12px;margin-top:10px">Factory reset clears all settings including WiFi credentials, schedules, AP configuration, and GPIO pin assignments.<br>Alternatively, hold the <strong>BOOT button</strong> (GPIO 0) for 5 seconds during normal operation to trigger a hardware factory reset.<br><br>✅ <strong>DS3231 RTC:</strong> Hardware real-time clock on GPIO21(SDA)/GPIO22(SCL) maintains time across power cycles.</p>
</div>
</main>
<div id="toast"></div>
<script>
...
void syncInternalRTC(time_t rawUtcEpoch) {
    ...
    if (rtcPresent) {
        ...
        rtc.adjust(dt);
        rtcTimeValid = true;
        Serial.println("DS3231 RTC updated from NTP");
    }
...
void handleBrowserTimeSync() {
    ...
    if (rtcPresent) {
        ...
        rtc.adjust(dt);
        rtcTimeValid = true;
        Serial.println("DS3231 RTC updated from browser");
    }
...
void setup() {
    ...
    if (!rtcInitialized && rtcPresent && rtcTimeValid) {
        ...
        if (now.year() >= 2020 && now.year() <= 2100) {
            ...
            rtcInitialized = true;
            timeSource = TIME_SOURCE_RTC;
            Serial.println("Using DS3231 RTC time as initial time source");
        }
    }
...
void loop() {
    ...
    if (rtcPresent && rtcInitialized && internalEpoch > 0) {
        if (timeHasElapsed(now, lastRTCDSync, RTC_SYNC_INTERVAL)) {
            ...
            DateTime dt(internalEpoch);
            rtc.adjust(dt);
            Serial.println("DS3231 RTC synced from internal time");
        }
    }
