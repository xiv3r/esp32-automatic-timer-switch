...
// Relay Configuration
#define NUM_RELAYS 16  // Expanded from 8 to 16
// ESP32 GPIO pins (adjust according to your wiring)
const int relayPins[NUM_RELAYS] = {
  32, 33, 25, 26, 27, 14, 12, 13,   // original 8
  15,  2,  4, 16, 17,  5, 18, 19    // added 8 – verify suitability for your board
};
const bool relayActiveLow = true; // Set true for active LOW relays

// Timer Schedule Structure (8 schedules per relay)
...
// HTML Pages – titles updated to 16‑channel, otherwise identical
...
void setup() {
    ...
    // Initialize relay configs with defaults (8 schedules each)
    for (int i = 0; i < NUM_RELAYS; i++) {
        for (int s = 0; s < 8; s++) {
...
// API Handlers – identical logic, just scales to NUM_RELAYS
void handleGetRelays() {
    DynamicJsonDocument doc(16384); // Increased for 16 relays with 8 schedules
...
void handleSaveRelay() {
    ...
    DynamicJsonDocument doc(4096); // Still fine for one relay's 8 schedules
    DeserializationError error = deserializeJson(doc, server.arg("plain"));
