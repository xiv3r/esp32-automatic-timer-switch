...
// ─── Multi-Year Reliability Constants ────────────────────────────────────────
static const unsigned long NVS_WRITE_MIN_INTERVAL   =   60000UL;   
static const unsigned long CRITICAL_SAVE_INTERVAL   = 3600000UL;   
static const unsigned long WIFI_STACK_RESET_INTERVAL = 86400000UL; 
static const unsigned long HEAP_DEFRAG_INTERVAL     = 43200000UL;  
...
// ─── Multi-Year Reliability Variables ────────────────────────────────────────
static unsigned long lastNVSWrite = 0;
static unsigned long lastCriticalSave = 0;
static unsigned long lastWiFiStackReset = 0;
static unsigned long lastHeapDefrag = 0;
static uint32_t totalUptimeDays = 0;
static unsigned long lastUptimeDayRollover = 0;
static unsigned long lastMillisForRollover = 0;
static size_t minFreeHeapEver = 0;
...
void checkAndCleanMemory() {
    ...
    if (now - lastHeapCheck > 300000) {
        ...
        if (freeHeap < minFreeHeapEver || minFreeHeapEver == 0) {
...
void saveGPIOConfig() {
    unsigned long now = millis();
    if (now - lastNVSWrite < NVS_WRITE_MIN_INTERVAL) {
        return;
    }
    lastNVSWrite = now;
    preferences.begin(NVS_NAMESPACE, false);
...
void setup() {
    ...
    ntpAsyncState = NTP_STATE_IDLE;
    lastNVSWrite = millis();
    lastWiFiStackReset = millis();
    lastHeapDefrag = millis();
    lastMillisForRollover = millis();
...
void loop() {
    ...
    unsigned long now = millis();
    if (now < lastMillisForRollover) {
        totalUptimeDays += 49;
    }
    lastMillisForRollover = now;
    if (now - lastUptimeDayRollover >= 86400000UL) {
        lastUptimeDayRollover = now;
        totalUptimeDays++;
    }    
    if (server.client()) {
        lastConnectionActivity = now;
    ...
    checkAndCleanMemory();
    if (now - lastHeapDefrag >= HEAP_DEFRAG_INTERVAL) {
        lastHeapDefrag = now;
        performMemoryCleanup();
    }
    if (now - lastWiFiStackReset >= WIFI_STACK_RESET_INTERVAL) {
        lastWiFiStackReset = now;
    ...
    if (wifiConnected) {
        bool doSync = false;        
        if (lastNTPSync == 0 && ntpAsyncState == NTP_STATE_IDLE) {
            doSync = true;
        } else if (ntpFailCount > 0 && now - lastNTPAttempt >= NTP_RETRY_INTERVAL && ntpAsyncState == NTP_STATE_IDLE) {
            doSync = true;
        } else if (now - lastNTPSync >= getNTPInterval() && ntpAsyncState == NTP_STATE_IDLE) {
            doSync = true;
        }        
        if (doSync) {
            tryNTPSync();
    ...
    if (now - lastCriticalSave >= CRITICAL_SAVE_INTERVAL) {
...
void setupWebServer() {
    ...
    server.on("/api/health", HTTP_GET, []() {
        ...
        doc["uptime_days"] = totalUptimeDays;
        doc["uptime_seconds"] = millis() / 1000UL;
        doc["free_heap"] = ESP.getFreeHeap();
        doc["min_free_heap_ever"] = minFreeHeapEver;
        doc["wifi_connected"] = wifiConnected;
        doc["wifi_reconnect_attempts"] = wifiReconnectAttempts;
        doc["ntp_failures"] = health.ntpFailures;
        doc["ntp_state"] = ntpAsyncState;
        doc["time_source"] = timeSource;
        doc["rtc_present"] = rtcPresent;
        doc["drift_compensation"] = driftCompensation;
