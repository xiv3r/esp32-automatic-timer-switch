...
void publishSystemStats() {
    ...
    if (timeSource == TIME_SOURCE_BROWSER) timeSourceStr = "browser";
...
function tick(){fetch('/api/time').then(r=>r.json()).then(d=>{document.getElementById('clk').textContent=d.time||'--:--:--';const w=document.querySelector('.wd'),t=document.querySelector('.td');if(w)w.className='dot '+(d.wifi?'g':'r');if(t){let tc='y';if(d.timeSource==='browser')tc='b';else if(d.timeSource==='rtc')tc='b';else tc='y';t.className='dot '+tc;}}).catch(()=>{});}
...
<small>When disabled, ESP32 runs in AP-only mode. Relay schedules using internal RTC/DS3231 will still work.</small>
</div>
<hr>
...
function toggleStation(){
  ...
  .then(r=>r.json()).then(d=>{
    btn.disabled=false;
    if(d.success){updateStaButton(willEnable);toast('WiFi Station '+(willEnable?'ON':'OFF'));const stationDiv=document.getElementById('stationConfig');if(stationDiv){stationDiv.style.opacity=willEnable?'1':'0.5';const inputs=stationDiv.querySelectorAll('input,button');inputs.forEach(inp=>inp.disabled=!willEnable)}loadWiFiStatus()}
...
<main>
<p class="ptitle">Time Settings</p>
<div class="card fcrd">
...
<small style="display:block;margin-top:12px;color:#90A4AE"><strong>DS3231 RTC Module</strong> is the primary time authority and maintains time across power cycles. Use <strong>Sync Browser</strong> to manually set time from this device.</small>
</div>
</main>
<div id="toast"></div>
<script>
function toast(m,ok=true){const t=document.getElementById('toast');t.textContent=m;t.className='show '+(ok?'ok':'er');clearTimeout(t._t);t._t=setTimeout(()=>t.className='',3000);}
function tick(){fetch('/api/time').then(r=>r.json()).then(d=>{document.getElementById('clk').textContent=d.time||'--:--:--';const w=document.querySelector('.wd'),t=document.querySelector('.td');if(w)w.className='dot '+(d.wifi?'g':'r');if(t){let tc='y';if(d.timeSource==='browser')tc='b';else if(d.timeSource==='rtc')tc='b';else tc='y';t.className='dot '+tc;}updateTimeStatus(d)}).catch(()=>{})}
function updateTimeStatus(d){
  const s=document.getElementById('timeStatus');s.style.display='block';
  let icon='',msg='',cls='ai';
  if(d.timeSource==='browser'){icon='\uD83C\uDF10';msg='Time: <strong>Browser Sync</strong> \u2014 Moderate';cls='aw'}
...
setInterval(tick,1000);tick();
fetch('/api/ntp').then(r=>r.json()).then(d=>{document.getElementById('gmt').value=d.gmtOffset||28800;document.getElementById('dst').value=d.daylightOffset||0}).catch(()=>{});
function save(){
  fetch('/api/ntp',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({gmtOffset:parseInt(document.getElementById('gmt').value),daylightOffset:parseInt(document.getElementById('dst').value)})})
  .then(r=>r.json()).then(d=>{if(d.success)toast('Time settings saved!');else toast('Failed: '+d.error,false)}).catch(()=>toast('Error',false));
}
...
void handleGetNTP() {
    ...
    char buffer[256];
    snprintf(buffer, sizeof(buffer),
        "{\"gmtOffset\":%d,\"daylightOffset\":%d,\"globalActiveMode\":%d}",
