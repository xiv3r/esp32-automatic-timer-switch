/*
===============================================================================
 *  ESP32S3 16-Channel Automatic Relay Timer Switch no NTP
 *  Author: Raff Alds
...
<small>When disabled, ESP32S3 runs in AP-only mode. Relay schedules using internal RTC/DS3231 will still work. You can still access this UI via the AP.</small>
</div>
...
function loadWiFiStatus() {
    fetch('/api/wifi').then(r=>r.json()).then(d=>{
        ...
        if(staEnabled && d.connected){
            ...
            s.innerHTML='<span style="font-weight:700">⛔ WiFi Station Disabled</span><br>ESP32S3 running in AP-only mode. Enable above to connect to WiFi networks.';
            s.className='alert aw';
        }
    }).catch(()=>{});
}
...
function toggleStation() {
    ...
    fetch('/api/wifi', {
        ...
        if(d.success) {
            ...
            if(!willEnable) {
                toast('Use DS3231 RTC or browser sync.', false);
            }
            const stationDiv = document.getElementById('stationConfig');
            if(stationDiv) {
...
function save(){
  ...
  })}).then(r=>r.json()).then(d=>{if(d.success)toast('Time settings saved!');else toast('Failed: '+d.error,false);})
  .catch(()=>toast('Error',false));
}
...
function loadSys(){
  fetch('/api/system').then(r=>r.json()).then(d=>{
    ...
    let tsrcStyle='';
    if(tsrc==='Browser')tsrcStyle='color:#1565C0';
