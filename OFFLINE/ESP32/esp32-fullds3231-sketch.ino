...
<small>When disabled, ESP32 runs in AP-only mode. Relay schedules using internal RTC/DS3231 will still work. You can still access this UI via the AP.</small>
</div>
...
function loadWiFiStatus() {
    fetch('/api/wifi').then(r=>r.json()).then(d=>{
        ...
        if(staEnabled && d.connected){
            ...
            s.innerHTML='<span style="font-weight:700">⛔ WiFi Station Disabled</span><br>ESP32 running in AP-only mode. Enable above to connect to WiFi networks.';
            s.className='alert aw';
        }
    }).catch(()=>{});
}
