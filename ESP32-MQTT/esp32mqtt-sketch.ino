...
private:
    ...
    State state = NTP_IDLE;
    WiFiUDP ntpUDP;
    byte packetBuffer[NTP_PACKET_SIZE];
    unsigned long startTime = 0;
    unsigned long timeoutMs = 0;
    const char* currentServer = nullptr;
    uint64_t result = 0;
    bool isComplete = false;
    uint8_t retryCount = 0;
    const uint8_t maxRetries = 3;
    unsigned long phaseStartTime = 0;
    const unsigned long SEND_TIMEOUT = 2000;
    const unsigned long RESPONSE_TIMEOUT = 3000;
    const unsigned long UDP_INIT_TIMEOUT = 500;
    uint8_t currentAttempt = 0;    
public:
    NonBlockingNTP() {}
    
    void start(const char* server, unsigned long timeoutMs = 5000) {
        if (state != NTP_IDLE) return;        
    ...
    void update() {
        ...
        unsigned long now = millis();
            switch (state) {
            case NTP_UDP_INIT:
                if (timeHasElapsed(now, phaseStartTime, UDP_INIT_TIMEOUT)) {
                    if (ntpUDP.begin(2390)) {
                        state = NTP_SEND_PACKET;
                        phaseStartTime = now;
                    } else {
                        if (retryCount++ < 3) {
                            phaseStartTime = now;
            ...
            case NTP_SEND_PACKET:
                if (timeHasElapsed(now, phaseStartTime, SEND_TIMEOUT)) {
                    memset(packetBuffer, 0, NTP_PACKET_SIZE);
                    packetBuffer[0] = 0b11100011; 
                    packetBuffer[1] = 0;
                    packetBuffer[2] = 6;
                    packetBuffer[3] = 0xEC; 
                    uint64_t nowEpoch = getCurrentEpoch();
                    uint64_t ntpNow = nowEpoch + NTP_EPOCH_OFFSET;
                    ...
                    packetBuffer[46] = 0;
                    packetBuffer[47] = 0;                   
                    ntpUDP.beginPacket(currentServer, NTP_PORT);
                    ntpUDP.write(packetBuffer, NTP_PACKET_SIZE);                    
                    if (ntpUDP.endPacket()) {
                        state = NTP_WAIT_RESPONSE;
                        phaseStartTime = now;
                    } else {
                        if (retryCount++ < 3) {
                            phaseStartTime = now;
            ...
            case NTP_WAIT_RESPONSE:
                if (timeHasElapsed(now, phaseStartTime, RESPONSE_TIMEOUT)) {
        ...
        if (state != NTP_IDLE && timeHasElapsed(now, startTime, timeoutMs + 5000)) {
            state = NTP_IDLE;
            ntpUDP.stop();
            isComplete = true;
            result = 0;
    ...
    bool isRunning() const { return state != NTP_IDLE; }
    bool isReady() const { return isComplete; }
    uint64_t getResult() const { return result; }
    bool isSuccess() const { return isComplete && result > 0; }
};
NonBlockingNTP ntpAsync;
uint64_t getNTP64Time(const char* server) {
    static NonBlockingNTP ntpInstance;
    static unsigned long lastStart = 0;
    static const unsigned long MAX_WAIT = 65000;     
    unsigned long now = millis();
    if (!ntpInstance.isRunning() && !ntpInstance.isReady()) {
        ntpInstance.start(server, NTP_TIMEOUT);
        lastStart = now;
    }
    ntpInstance.update();
    if (ntpInstance.isReady()) {
        uint64_t result = ntpInstance.getResult();
        ntpInstance.stop();
        return result;
    }
    if (timeHasElapsed(now, lastStart, MAX_WAIT)) {
        ntpInstance.stop();
        return 0;
    }
    return 0;
...
void tryNTPSync() {
    ...
    if (ntpAsync.isRunning()) {
        ntpAsync.update();
        if (ntpAsync.isReady()) {
            uint64_t ntpTime = ntpAsync.getResult();
            ntpAsync.stop();            
            if (ntpTime > 1577836800ULL) {
                syncInternalRTC(ntpTime);
                ntpFailCount = 0;
                ...
                health.ntpFailures++;
                ntpServerIndex = (ntpServerIndex + 1) % NUM_NTP_SERVERS;
                ntpAsync.start(NTP_SERVERS[ntpServerIndex], NTP_TIMEOUT);
                lastNTPAttempt = now;
                return;
            }
        }
