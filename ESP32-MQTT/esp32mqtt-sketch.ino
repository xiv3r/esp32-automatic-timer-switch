...
void handleSaveRelay() {
    ...
    DynamicJsonDocument doc(4096);   // FIX: was StaticJsonDocument<5120> (5KB stack frame in an ~8KB web-server task); heap-allocated, 2994/4096 pool used worst-case, freed at function exit
    DeserializationError err = deserializeJson(doc, server.arg("plain"));
    if (err) {
        server.send(400, "application/json", "{\"success\":false,\"error\":\"Bad JSON\"}"); 
        return;
