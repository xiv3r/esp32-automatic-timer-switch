...
bool SelfHealingSystem::recoverNTP() {
    ...
    for (uint8_t attempt = 0; attempt < NUM_NTP_SERVERS; attempt++) {
        ...
        if (timeClient.forceUpdate()) {
            time_t rawUtcEpoch32 = timeClient.getEpochTime();
            uint64_t rawUtcEpoch = time32To64(rawUtcEpoch32);
            if (VALID_UNIX_TIME_64(rawUtcEpoch)) {
