        ...
        for (int s = 0; s < 8; s++) {
            ...
            if (start == stop) {
                ...
                if (cur >= start) {
                    if (monthMask && !(monthMask & (1 << cachedMonth))) continue;
                    if (monthDayMask && !(monthDayMask & (1 << (cachedMonthDay - 1)))) continue;
                    if (relayConfigs[i].schedule.days[s] & (1 << ti->tm_wday)) {
                        hasActive = true;
                        break;
                    }
                } else if (cur < stop) {
                    int hoursSinceStart = (24 * 3600 - start) + cur;
                    int daysSinceStart = hoursSinceStart / 86400;
                    if (daysSinceStart > 6) daysSinceStart = 6;                    
                    bool dayMatches = false;
                    for (int d = 0; d <= daysSinceStart; d++) {
                        uint8_t checkDay = (ti->tm_wday + 7 - d - 1) % 7;
                        if (!(relayConfigs[i].schedule.days[s] & (1 << checkDay))) continue;
                        uint64_t checkEpoch = epoch - (d * 86400ULL);
