...
//  Year 2106+ Support
// =============================================================================
#define MAX_UNIX_TIME_64 16725225600ULL
...
static bool     mqttPulseActive[MAX_RELAYS] = {false};
static uint32_t mqttPulseEndMs[MAX_RELAYS]  = {0};
...
static uint64_t uptimeTotalMillis = 0;
inline void updateUptime() {
    uint32_t now = millis();
    uptimeTotalMillis += (uint32_t)(now - uptimeLastMillis);
    uptimeLastMillis = now;
}
inline uint64_t getUptimeSeconds() {
    return uptimeTotalMillis / 1000ULL;
}
...
void saveRTCState() {
    ...
    preferences.putULong64("rtcEpoch", internalEpoch);
    preferences.end();
}

void loadRTCState() {
    preferences.begin(NVS_NAMESPACE, true);
    uint64_t saved = preferences.getULong64("rtcEpoch", 0);
    preferences.end();
    if (saved == 0) saved = sysConfig.last_rtc_epoch;
    if (VALID_UNIX_TIME_64(saved)) {
        internalEpoch            = saved;
        driftCompensation        = 1.0f;
        internalMillisAtLastSync = millis();
        rtcMicrosAtLastSync      = micros();
...
//  Self-Healing System 
// =============================================================================
static uint32_t calculateCriticalChecksumOf(const CriticalRelayState& st) {
    uint32_t sum = 0;
    for (int i = 0; i < MAX_RELAYS; i++) {
        sum += st.relayStates[i] ? (1 << (i % 32)) : 0;
        sum += st.manualOverrides[i] ? (1 << ((i+16) % 32)) : 0;
    }
    return sum ^ st.timestamp;
}
uint32_t calculateCriticalChecksum() {
    return calculateCriticalChecksumOf(criticalState);
}
...
bool SelfHealingSystem::restoreCriticalState() {
    CriticalRelayState saved = {};   
    preferences.begin(NVS_NAMESPACE, true);
    size_t len = preferences.getBytes("criticalState", &saved, sizeof(saved));
    preferences.end();
    bool blobValid = (len == sizeof(saved))
                     && (saved.magic == 0xDEADBEEF)
                     && (saved.checksum == calculateCriticalChecksumOf(saved));
    if (blobValid) {
        for (int i = 0; i < gpioConfig.count && i < MAX_RELAYS; i++) {
            relayConfigs[i].manualOverride = saved.manualOverrides[i];
            if (saved.manualOverrides[i]) {
                relayConfigs[i].manualState = saved.relayStates[i];
                setRelayOutput(i, saved.relayStates[i]);
                lastRelayOutputs[i] = saved.relayStates[i];
            }
        }
        criticalStateInitialized = true;
        updateScheduleCache();
...
void publishHADiscovery() {
    ...
    if (deviceName.isEmpty()) deviceName = "ESP32_Relay16";    
    for (int i = 0; i < getActiveRelayCount(); i++) {
...
void handleScheduleSet(uint8_t relayIndex, const char* jsonPayload) {
    ...
    for (JsonObject sch : schedules) {
        ...
        if (sch.containsKey("start")) {
            ...
            relayConfigs[relayIndex].schedule.startHour[s]   = constrain(h, 0, 23);
            relayConfigs[relayIndex].schedule.startMinute[s] = constrain(m, 0, 59);
            relayConfigs[relayIndex].schedule.startSecond[s] = constrain(sec, 0, 59);
        }
        if (sch.containsKey("stop")) {
            String stop = sch["stop"];
            int h = 0, m = 0, sec = 0;
            sscanf(stop.c_str(), "%d:%d:%d", &h, &m, &sec);
            relayConfigs[relayIndex].schedule.stopHour[s]   = constrain(h, 0, 23);
            relayConfigs[relayIndex].schedule.stopMinute[s] = constrain(m, 0, 59);
            relayConfigs[relayIndex].schedule.stopSecond[s] = constrain(sec, 0, 59);
        }
        if (sch.containsKey("days")) {
            uint8_t rawDays = (uint8_t)((int)sch["days"] & 0x7F);
            relayConfigs[relayIndex].schedule.days[s] = (rawDays == 0) ? 0x7F : rawDays;
        }
        if (sch.containsKey("month_days"))
            relayConfigs[relayIndex].schedule.monthDays[s] = (uint32_t)sch["month_days"];
    ...
    if (topicStr.startsWith(base + "/relay/")) {
        ...
        if (command == "set") {
            ...
            scheduleActiveCache[relayIndex] = false;
                mqttPulseActive[relayIndex] = false;   
            ...
            if (duration > 0 && duration <= 30000) {
                ...
                mqttPulseActive[relayIndex] = true;
                mqttPulseEndMs[relayIndex] = millis() + (uint32_t)duration;
                responseCache.valid = false;
            }
        }
...
void handleMqtt() {
    ...
    if (!mqttInitialized) {
        ...
        mqttWiFiClient.setTimeout(3000UL);
        mqttClient->setSocketTimeout(5);
        mqttClient->setServer(mqttFullConfig.broker_ip, mqttFullConfig.broker_port);
        mqttClient->setCallback(mqttFullCallback);
        uint16_t ka = mqttFullConfig.keepalive;
        if (ka < 10 || ka > 3600) ka = 60;
        mqttClient->setKeepAlive(ka);
        mqttInitialized = true;
    }
    for (int i = 0; i < MAX_RELAYS; i++) {
        if (mqttPulseActive[i] && (int32_t)(millis() - mqttPulseEndMs[i]) >= 0) {
            mqttPulseActive[i] = false;
            if (i < gpioConfig.count) {
                relayConfigs[i].manualState = false;
                setRelayOutput(i, false);
                lastRelayOutputs[i] = false;
                if (mqttClient && mqttClient->connected()) {
                    mqttPublish(mqttGetRelayStateTopic(i).c_str(), "OFF", true);
                }
                criticalStateDirty = true;
                responseCache.valid = false;
            }
    ...
    uint32_t statInterval = mqttFullConfig.status_interval;   
    if (statInterval < 5 || statInterval > 3600) statInterval = 30;
    if (timeHasElapsed(now, lastMqttStatusPublish, statInterval * 1000UL)) {
...
void loadMqttFullConfig() {
    ...
    if (len != sizeof(MqttFullConfig) || mqttFullConfig.magic != MQTT_FULL_MAGIC) {
        ...
        snprintf(mqttFullConfig.client_id, 40, "ESP32_Relay16_%06X", (uint32_t)(ESP.getEfuseMac() & 0xFFFFFF));
        strcpy(mqttFullConfig.base_topic, "esp32relay16");
...
<div class="fg"><label>Client ID</label><input type="text" id="clientId" placeholder="ESP32_Relay16" required></div>
...
<div class="fg"><label>Base Topic</label><input type="text" id="topic" placeholder="esp32relay16" required></div>
...
// =============================================================================
static uint8_t getNTPServerCount() {
    return (uint8_t)(NUM_NTP_SERVERS + (sysConfig.ntp_server[0] ? 1 : 0));
}
static const char* getNTPServer(uint8_t idx) {
    if (sysConfig.ntp_server[0]) {
        if (idx == 0) return sysConfig.ntp_server;
        return NTP_SERVERS[(idx - 1) % NUM_NTP_SERVERS];
    }
    return NTP_SERVERS[idx % NUM_NTP_SERVERS];
}
void startNTPRequest(const char* server) {
...
void processNTPResponse() {
    ...
    if (packetSize >= NTP_PACKET_SIZE) {
        ...
        if (mode == 4 && li != 3) {
            ...
            uint64_t unixA = (uint64_t)secs - NTP_EPOCH_OFFSET;
            uint64_t unixB = (uint64_t)secs + 0x100000000ULL - NTP_EPOCH_OFFSET;
            uint64_t ref = (rtcInitialized && internalEpoch > MIN_UNIX_TIME_64)
                           ? internalEpoch : 1800000000ULL;
            uint64_t dA = (unixA > ref) ? (unixA - ref) : (ref - unixA);
            uint64_t dB = (unixB > ref) ? (unixB - ref) : (ref - unixB);
            uint64_t unixEpoch = (dA <= dB) ? unixA : unixB;
...
void updateNTPSync() {
    switch (ntpAsyncStage) {
        case 0: 
            if (ntpRetryCount < getNTPServerCount()) {
                startNTPRequest(getNTPServer(ntpAsyncCurrentServer));
            } else {
                ntpFailCount++;
                health.ntpFailures++;
                ntpRetryCount = 0;
        ...
        case 2: 
            ...
            if (ntpReceived) {
                ...
                ntpServerIndex = (ntpAsyncCurrentServer + 1) % getNTPServerCount();
            }
            ntpAsyncCurrentServer = (ntpAsyncCurrentServer + 1) % getNTPServerCount();
            ntpRetryCount = 0;
            ntpAsyncStage = 0;
            ntpAsyncState = NTP_STATE_IDLE;
            break;            
...
void processRelaySchedules() {
    ...
    int currentSeconds = ti->tm_hour * 3600 + ti->tm_min * 60 + ti->tm_sec;    
    int cur = currentSeconds;
    uint8_t todayBit = (1 << currentWeekday); 
    ...
    for (int i = 0; i < gpioConfig.count; i++) {
        ...
        if (lastDebouncedState[i] != shouldBeOn || lastRelayOutputs[i] != shouldBeOn) {
...
void loop() {
    ...
    if (criticalStateDirty && timeHasElapsed(now, lastCriticalSave, 60000)) {
