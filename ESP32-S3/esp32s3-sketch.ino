...
<small style="display:block;margin-top:12px;color:#90A4AE">Use <strong>Sync Browser</strong> when NTP servers are unreachable. NTP will automatically override browser time when available. DS3231 hardware RTC maintains time across power cycles and auto-syncs every 10 minutes.</small>
</div>
</main>
<div id="toast"></div>
<script>
    ...
    if (rtcInitialized && internalEpoch > 0) {
        ...
        if (elapsedMicros > 10000000UL) {
            ...
            if (measuredRate > 0.9f && measuredRate < 1.1f) {
                ntpDriftHistory[ntpDriftIdx] = measuredRate;
                ntpDriftIdx = (ntpDriftIdx + 1) % 4;  
                float avgMeasured = 0;
                uint8_t samples = 4;
