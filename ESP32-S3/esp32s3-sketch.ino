 ...
 *  Author: Raff Alds
 *  Profile: https://www.github.com/xiv3r
 *  License: GPLv3
===============================================================================
*/
...
void immediateDS3231Sync() {
    if (!rtcPresent || !rtcInitialized || internalEpoch <= 0) return;
...
void syncInternalRTCFromDS3231() {
    ...
    if (nowRTC.year() >= 2020 && nowRTC.year() <= 2100) {
        time_t rtcEpoch = nowRTC.unixtime();
        if (VALID_UNIX_TIME(rtcEpoch)) {
...
bool SelfHealingSystem::liveReconfigureMDNS() {
    ...
    if (!mdnsStarted) {
        if (MDNS.begin(mdnsHostname)) {
            ...
            MDNS.addServiceTxt("http", "tcp", "version", "v10");
...
.months{display:flex;gap:2px;margin-top:5px;flex-wrap:wrap}
.month{width:32px;height:22px;border-radius:3px;border:1px solid #CFD8DC;display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:600;cursor:pointer;background:#FAFAFA;transition:.15s;user-select:none}
.month:hover{border-color:#FF8A65;background:#FBE9E7}
.month.on{background:#E64A19;color:#fff;border-color:#E64A19}
...
@media(max-width:500px){.grid{grid-template-columns:1fr}.ibar{grid-template-columns:1fr}.input-row{flex-direction:column}.day{width:24px;height:22px;font-size:10px}.mday{width:22px;height:20px;font-size:9px}.month{width:28px;height:20px;font-size:8px}}
)css";
...
function monthMaskToStr(mm){
  ...
  if(mm===0) return 'None';
  let s='';
  for(let i=0;i<12;i++) if(mm&(1<<i)) s+=MONTHS[i]+',';
  return s.replace(/,$/,'')||'None';
}
...
function nightBadge(sc){
  ...
  const mos=monthMaskToStr(sc.months||0x0FFF);
  let info=ds;
  if(mos) info+=' | '+mos;
  if(ms) info+=' | Days:'+ms;
...
<div class="months" id="month_${i}_${s}">`;
      for(let m=0;m<12;m++){
        const mask = 1<<m;
        html+=`<div class="month${(monthBits&mask)?' on':''}" onclick="toggleMonth(${i},${s},${m})" title="${MONTHS[m]}">${MONTHS[m]}</div>`;
      }
      html+=`</div>
...
function updateTimeStatus(d){
  ...
  let rtcInfo = '';
  if(d.rtcPresent){
    rtcInfo = '<br><small>\u2705 DS3231 RTC detected on GPIO 8/9';
...
void updateScheduleCache() {
    ...
    for (int i = 0; i < gpioConfig.count; i++) {
        ...
        for (int s = 0; s < 8; s++) {
            ...
            if (monthMask != MONTH_ALL && !(monthMask & (1 << cachedMonth))) continue;
            uint32_t monthDayMask = relayConfigs[i].schedule.monthDays[s];
            if (monthDayMask == 0) continue;
            if (!(monthDayMask & (1 << (cachedMonthDay - 1)))) continue;                        
...
void processRelaySchedules() {
    ...
    for (int i = 0; i < gpioConfig.count; i++) {
        ...
        for (int s = 0; s < 8; s++) {
            ...
            uint16_t monthMask = relayConfigs[i].schedule.months[s];
            if (monthMask == 0) continue;
            if (monthMask != MONTH_ALL && !(monthMask & (1 << month))) continue;
            uint32_t monthDayMask = relayConfigs[i].schedule.monthDays[s];
            if (monthDayMask == 0) continue;
...
void setup() {
    ...
    for (int i = 0; i < MAX_RELAYS; i++) {
        for (int s = 0; s < 8; s++) {
            ...
            relayConfigs[i].schedule.monthDays[s] = 0xFFFFFFFF;  
...
void handleAddGPIO() {
    ...
    for (int s = 0; s < 8; s++) {
        ...
        relayConfigs[gpioConfig.count].schedule.monthDays[s] = 0xFFFFFFFF;  
...
void handleDeleteGPIO() {
    ...
    for (int s = 0; s < 8; s++) {
        ...
        relayConfigs[gpioConfig.count - 1].schedule.monthDays[s] = 0xFFFFFFFF;  
