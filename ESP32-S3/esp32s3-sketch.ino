...
function loadSys(){
  fetch('/api/system').then(r=>r.json()).then(d=>{
    ...
    document.getElementById('sds3231master').textContent = d.ds3231Master ? '✅ Enabled (Master)' : '❌ Disabled (Internal RTC)';
    if(d.ds3231Master) document.getElementById('sds3231master').style.color='#2E7D32';
    else document.getElementById('sds3231master').style.color='#C62828';
  }).catch(()=>{});
}
