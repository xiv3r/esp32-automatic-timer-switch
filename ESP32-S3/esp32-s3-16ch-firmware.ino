/*
============================================================
 *  ESP32-S3 16-Channel Relay Smart Switch — Firmware v4.1
...
#define EEPROM_VERSION 3        // v3 main layout preserved — no changes
...
static const unsigned long WIFI_SCAN_TIMEOUT    =   10000UL; // 10 s scan timeout

// ─── mDNS settings ────────────────────────────────────────────────────────────
#define MDNS_HOSTNAME_DEFAULT "esp32-16ch-relay"
static const unsigned long MDNS_RESTART_DELAY = 2000UL;  // 2s after AP restart
...
// ESP32-S3-safe GPIO pins (avoid strapping pins 0/3/45/46, USB 19/20,
// internal SPI/PSRAM 26-33, and reserved 48 on most devkits)
...
// v3 SystemConfig — NOT changed (keeps migration path intact)
struct SystemConfig {
    uint16_t magic;
    uint8_t  version;
    char     sta_ssid[32];
    ...
    float    rtc_drift;
    char     hostname[32];   // kept for backward compatibility (unused)
};

// v4 ExtConfig — completely independent of main layout
struct ExtConfig {
...
char ap_password[32] = "ESP32-S3-admin";
...
// ─────────────────────────────────────────────────────────────────────────────
//  SHARED CSS (Unchanged - Preserved for web UI compatibility)
// ─────────────────────────────────────────────────────────────────────────────
...
.ctitle{font-weight:700;font-size:15px;cursor:pointer}
.ctitle:hover{color:#1565C0;text-decoration:underline dotted #90CAF9}
...
// ─────────────────────────────────────────────────────────────────────────────
//  HTML Pages (Unchanged - Preserved for web UI compatibility)
// ─────────────────────────────────────────────────────────────────────────────
// [All HTML content preserved exactly as in original - omitted for brevity]
// The original index_html, wifi_html, ntp_html, ap_html, system_html remain unchanged

const char index_html[] PROGMEM = R"raw(<!DOCTYPE html>
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Relays — ESP32-S3 Timer Switch</title>
<link rel="stylesheet" href="/style.css"></head><body>
<header>
<span class="logo">&#x26A1; 16-Ch Relay</span>
<nav>
...
function editName(ri){
  const span=document.getElementById('name_'+ri);
  const current=span.textContent;
  const input=document.createElement('input');
  input.type='text';
  input.value=current;
  input.maxLength=15;
  input.style.cssText='font-weight:700;font-size:15px;border:1px solid #1565C0;border-radius:5px;padding:2px 6px;width:140px;background:#fff';
  
  const save=()=>{
    const newName=input.value.trim()||('Relay '+(ri+1));
    ...
    fetch('/api/relay/name',{
      ...
      body:JSON.stringify({relay:ri,name:newName})
    }).then(r=>r.json()).then(d=>{
      if(d.success){
        relays[ri].name=newName;
        toast('Name saved');
      }else{
...
<div class="card-hdr">
<span class="ctitle" id="name_${i}" ondblclick="editName(${i})" title="Double-click to edit">${r.name||'Relay '+(i+1)}</span>
...
<input type="time" step="1" id="et_${i}_${s}" value="${toTS(sc2.stopHour,sc2.stopMinute,sc2.stopSecond)}" onchange="uf(${i},${s},'stop',this.value)">
    ...
    html+=`</div><button class="btn bsave" onclick="save(${i})">&#x1F4BE; Save Relay ${i+1}</button></div>`;
...
function mc(ri,state){
  ...
  .then(r=>r.json()).then(d=>{if(d.success){toast('Relay '+(ri+1)+' '+(state?'ON':'OFF'));load();}else toast('Failed',false);})
...
function ra(ri){
  ...
  .then(r=>r.json()).then(d=>{if(d.success){toast('Relay '+(ri+1)+' \u2192 Auto');load();}else toast('Failed',false);})
  .catch(()=>toast('Error',false));
}
function save(ri){
  busy=true;
  ...
  .then(r=>r.json()).then(d=>{busy=false;if(d.success)toast('Relay '+(ri+1)+' saved!');else toast('Save failed',false);})
...
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>WiFi — ESP32-S3 Timer Switch</title>
<link rel="stylesheet" href="/style.css"></head><body>
<header>
<span class="logo">&#x26A1; 16-Ch Relay</span>
<nav>
...
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Time — ESP32-S3 Timer Switch</title>
<link rel="stylesheet" href="/style.css"></head><body>
<header>
<span class="logo">&#x26A1; 16-Ch Relay</span>
<nav>
...
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>AP — ESP32-S3 Timer Switch</title>
<link rel="stylesheet" href="/style.css"></head><body>
<header>
<span class="logo">&#x26A1; 16-Ch Relay</span>
<nav>
...
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>System — ESP32-S3 Timer Switch</title>
<link rel="stylesheet" href="/style.css"></head><body>
<header>
<span class="logo">&#x26A1; 16-Ch Relay</span>
<nav>
...
// =============================================================================
//  RTC FUNCTIONS (Fixed drift calculation)
// =============================================================================

time_t getCurrentEpoch() {
    if (!rtcInitialized || internalEpoch == 0) return 0;
    unsigned long elapsed = millis() - internalMillisAtLastSync;
    // Fixed: Use proper floating point calculation to avoid overflow
...
void syncInternalRTC() {
    ...
    if (rtcInitialized && internalEpoch > 0) {
        ...
        if (elapsedMs > 60000UL) {
            ...
            Serial.printf("[RTC] Drift adjusted: %.6f (measured: %.6f)\n", 
                         driftCompensation, measuredRate);
        }
    }
...
void loadRTCState() {
    if (sysConfig.last_rtc_epoch > 1000000000UL &&
        ...
        if (driftCompensation < 0.90f || driftCompensation > 1.10f) {
            ...
            Serial.println("[RTC] Drift out of bounds, reset to 1.0");
        }
        ...
        Serial.println("[RTC] No valid epoch in NVS");
    }
}

// =============================================================================
//  NTP FUNCTIONS (Improved retry logic)
// =============================================================================
...
void tryNTPSync() {
    ...
    // Try all servers in rotation until success or full cycle
    for (uint8_t attempt = 0; attempt < NUM_NTP_SERVERS; attempt++) {
        ...
        Serial.printf("[NTP] Attempt %d/%d: %s\n", 
                     attempt + 1, NUM_NTP_SERVERS, NTP_SERVERS[idx]);
        ...
        if (timeClient.forceUpdate()) {
            syncInternalRTC();
            ntpServerIndex = idx;  // Remember working server
            ntpFailCount = 0;
            synced = true;
            Serial.println("[NTP] Sync successful!");
            break;
        } else {
            Serial.printf("[NTP] Failed: %s\n", NTP_SERVERS[idx]);
        }
    ...
    if (!synced) {
        ...
        Serial.printf("[NTP] All servers failed (%u consecutive failures)\n", ntpFailCount);
    }
}

// =============================================================================
//  NON-BLOCKING WiFi STA RECONNECT (Fixed state machine)
// =============================================================================
...
// =============================================================================
//  RELAY FUNCTIONS (Added centralized update function)
// =============================================================================
...
void updateRelayOutputs() {
    for (int i = 0; i < NUM_RELAYS; i++) {
        ...
        if (relayConfigs[i].manualOverride) {
            ...
            // State already calculated in processRelaySchedules
            // This function just applies the physical output
            // We'll read the current pin state instead of recalculating
...
void restartAP() {
    ...
    if (strlen(sysConfig.ap_password) > 0) {
        result = WiFi.softAP(sysConfig.ap_ssid, sysConfig.ap_password, ch, hidden);
    } else {
        result = WiFi.softAP(sysConfig.ap_ssid, NULL, ch, hidden);
    }
    
    if (result) {
        Serial.printf("[AP] Started: SSID=%s ch=%u hidden=%u IP=%s\n",
    ...
    // Restart mDNS with potentially new hostname
    scheduleMDNSRestart();
}

// =============================================================================
//  mDNS FUNCTIONS (Improved reliability)
// =============================================================================

void startMDNS() {
    if (mdnsStarted) return;
    
    // Build hostname from AP SSID (sanitized) or use saved hostname
    ...
    Serial.printf("[mDNS] Starting with hostname: %s.local\n", mdnsHostname);
    
    // Add retry logic for mDNS startup
    int retries = 3;
    while (retries-- > 0) {
        if (MDNS.begin(mdnsHostname)) {
            // Add service advertisements
            MDNS.addService("http", "tcp", 80);
            MDNS.addServiceTxt("http", "tcp", "model", "ESP32-S3-16CH-Relay");
            MDNS.addServiceTxt("http", "tcp", "version", "v4.1");
            MDNS.addServiceTxt("http", "tcp", "channels", "16");
            
            mdnsStarted = true;
            Serial.printf("[mDNS] Started successfully - access at http://%s.local\n", mdnsHostname);
            return;
        }
        delay(100);
    }
    
    Serial.println("[mDNS] Failed to start after retries");
    mdnsStarted = false;
}
...
void scheduleMDNSRestart() {
    ...
    Serial.println("[mDNS] Restart scheduled");
}
...
// =============================================================================
//  SETUP (Fixed initialization order and relay safety)
// =============================================================================

void setup() {
    Serial.begin(115200);
    Serial.println("\n[Boot] ESP32-S3 16-Ch Relay v4.1 (Fixed)");
...
// =============================================================================
//  LOOP  — fully non-blocking (Fixed WiFi and scan handling)
// =============================================================================
...
void loop() {
    ...
    if (wcsState == WCS_PENDING) {
        ...
        if (status == WL_CONNECTED) {
            ...
            // Trigger NTP sync after successful connection
            lastNTPSync = 0;
            lastNTPAttempt = 0;
            ...
            if (wifiReconnectAttempts >= MAX_RECONNECT) {
                wifiGiveUpUntil = now + 300000UL;  // 5 minute backoff
                wifiReconnectAttempts = 0;
    ...
    if (now - lastWiFiCheck >= WIFI_CHECK_INTERVAL) {
        ...
        if (wifiConnected && !currentlyConnected) {
            ...
            Serial.printf("[WiFi] Connection restored. IP=%s\n", 
                         WiFi.localIP().toString().c_str());
            lastNTPSync = 0;  // Trigger NTP sync
        }
    }
...
// =============================================================================
//  SCHEDULE ENGINE  (Fixed overnight and always-ON edge cases)
// =============================================================================
...
void processRelaySchedules() {
    ...
    for (int i = 0; i < NUM_RELAYS; i++) {
        ...
        for (int s = 0; s < 8; s++) {
            ...
            // Fixed: Proper handling of all schedule cases
            if (start == stop) {
                // Always ON
                on = true;
                break;  // Once always-on is found, no need to check others
            } else if (start < stop) {
...
// =============================================================================
//  CONFIGURATION  (Fixed validation and corruption handling)
// =============================================================================
...
void initDefaults() {
    ...
    strcpy(sysConfig.ap_password, "ESP32-S3-admin");
...
void loadConfiguration() {
    ...
    if (len != sizeof(SystemConfig) || sysConfig.magic != EEPROM_MAGIC) {
        configValid = false;
        Serial.println("[CFG] Invalid or missing main config");
    }
    ...
    if (sysConfig.version == EEPROM_VERSION) {
        len = preferences.getBytes("relayConfigs", relayConfigs, sizeof(RelayConfig) * NUM_RELAYS);
        if (len != sizeof(RelayConfig) * NUM_RELAYS) {
            Serial.println("[CFG] Relay configs corrupted, resetting");
            for (int i = 0; i < NUM_RELAYS; i++) {
                memset(&relayConfigs[i], 0, sizeof(RelayConfig));
                snprintf(relayConfigs[i].name, 16, "Relay %d", i + 1);
            }
        }
        Serial.println("[CFG] Loaded v3 configuration");
    } else {
        Serial.printf("[CFG] Unknown version %u — resetting\n", sysConfig.version);
        preferences.end();
        initDefaults();
        preferences.begin(NVS_NAMESPACE, true);
    }
...
void saveConfiguration() {
    ...
    if (written != sizeof(SystemConfig)) {
        Serial.println("[CFG] Warning: Incomplete write of sysConfig");
    }
    ...
    if (written != sizeof(RelayConfig) * NUM_RELAYS) {
        Serial.println("[CFG] Warning: Incomplete write of relayConfigs");
    }
...
void saveExtConfig() {
    ...
    preferences.end();
    Serial.println("[EXT] Saved to NVS");
}
...
// =============================================================================
//  API HANDLERS (Fixed JSON handling and memory leaks)
// =============================================================================

void handleGetRelays() {
    DynamicJsonDocument doc(16384);  // Increased size for 16 relays with schedules
...
void handleWiFiScanResults() {
    ...
    for (int i = 0; i < scanResultCount && i < 30; i++) {
        ...
        if (ssid.length() == 0) continue;  // Skip hidden networks without SSID
