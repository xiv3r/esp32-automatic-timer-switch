/*
============================================================
 *  ESP32-C3 16-Channel Relay Smart Switch — Firmware v4.1-C3
...
// ─── mDNS settings ────────────────────────────────────────────────────────────
#define MDNS_HOSTNAME_DEFAULT "esp32c3-16ch-relay"
...
// AP copies
char ap_ssid[32]     = "ESP32-C3_16CH_Timer_Switch";
char ap_password[32] = "ESP32-C3-admin";
...
void startMDNS() {
    ...
    while (retries-- > 0) {
        if (MDNS.begin(mdnsHostname)) {
            ...
            MDNS.addServiceTxt("http", "tcp", "model", "ESP32-C3-16CH-Relay");
...
void setup() {
    Serial.begin(115200);
    Serial.println("\n[Boot] ESP32-C3 16-Ch Relay v4.1 (Fixed)");
    ...
    // HIGH during boot. Active-low relays naturally satisfy this (pulled HIGH).
    // If using active-high relays on strapping pins, add 10kΩ pull-ups.
    for (int i = 0; i < NUM_RELAYS; i++) {
...
void initDefaults() {
    ...
    strcpy(sysConfig.ap_ssid,     "ESP32-C3_16CH_Timer_Switch");
    strcpy(sysConfig.ap_password, "ESP32-C3-admin");
