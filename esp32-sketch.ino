...
static unsigned long wifiPausedAt   = 0;  
...
unsigned long mdnsRestartRequestedAt = 0; 
...
void loadRTCState() {
    if (sysConfig.last_rtc_epoch > 1000000000UL &&
        sysConfig.last_rtc_epoch < 4294967295UL) {
        internalEpoch            = sysConfig.last_rtc_epoch;
        driftCompensation        = 1.00f;  
...
bool SelfHealingSystem::liveReconfigureAP() {
    ...
    bool apDown = (WiFi.softAPIP().toString() == "0.0.0.0");
    if (apDown) {
...
bool SelfHealingSystem::recoverNTP() {
    ...
    for (uint8_t attempt = 0; attempt < NUM_NTP_SERVERS; attempt++) {
        ...
        if (rawUtcEpoch > 1000000000UL && rawUtcEpoch < 4294967295UL) {
...
<small style="display:block;margin-top:12px;color:#90A4AE">Use <strong>Sync Browser</strong> when offline or NTP servers are unreachable. <strong>NTP</strong> will automatically override browser time when available. <strong>DS3231 RTC</strong> maintains time across power cycles when available.</small>
</div>
</main>
<div id="toast"></div>
<script>
...
function syncFromBrowser(){
  ...
  .then(d => {
    ...
    if (d.success) {
      ...
      if(d.drift) msg += ' Drift: '+d.drift.toFixed(4);
      toast(msg);
      tick();
    } else {
...
// =============================================================================
void syncInternalRTC(time_t rawUtcEpoch) {
    if (rawUtcEpoch < 1000000000UL || rawUtcEpoch > 4294967295UL) return;    
...
void handleBrowserTimeSync() {
    ...
    if (browserUtcEpoch < 1577836800 || browserUtcEpoch > 4294967295) {
        server.send(400, "application/json", 
...
void beginWiFiConnect() {
    ...
    if (wifiGaveUpAt > 0 && (millis() - wifiGaveUpAt) < 300000UL) return;  
...
void loop() {
    ...
    if (mdnsRestartScheduled && (now - mdnsRestartRequestedAt) >= MDNS_RESTART_DELAY) {  
    ...
    if (wifiPausedForScan && (now - wifiPausedAt) >= WIFI_PAUSE_DURATION) {  
        wifiPausedForScan = false;
        wifiReconnectAttempts = 0;
        wifiGaveUpAt = 0;  
    }
    ...
    if (now - lastWiFiCheck >= WIFI_CHECK_INTERVAL) {
        ...
        if (!wifiPausedForScan) {
            if (!wifiConnecting && !wifiConnected && strlen(sysConfig.sta_ssid) > 0 &&
                (wifiGaveUpAt == 0 || (now - wifiGaveUpAt) >= 300000UL)) {  
    ...
    if (criticalStateDirty && now - lastCriticalSave >= 300000) {
...
void handleGetSystem() {
    ...
    unsigned long uptimeSecs = (bootEpoch > 0 && nowEpoch > bootEpoch)
                               ? (unsigned long)(nowEpoch - bootEpoch)
                               : millis() / 1000UL;  
...
void handleGetGPIOConfig() {
    ...
    // Change GPIO PIN
    int validPins[] = {15, 2, 4, 16, 17, 5, 18, 19, 21, 3, 1, 22, 23, 13, 14, 27, 26, 25, 33, 32};    
    for (int p : validPins) {
        available.add(p);
    }    
    String resp;
