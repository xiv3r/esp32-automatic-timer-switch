...
#define MONTHDAY_ALL    0x7FFFFFFFUL
...
void initScheduleDefaults(int relayIndex) {
    ...
    for (int s = 0; s < 8; s++) {
        ...
        relayConfigs[relayIndex].schedule.monthDays[s] = MONTHDAY_ALL;   
...
bool SelfHealingSystem::liveReconfigureMDNS() {
    ...
    if (!mdnsStarted) {
        if (MDNS.begin(mdnsHostname)) {
            ...
            MDNS.addServiceTxt("http", "tcp", "version", "v13");  
...
const ALL_MONTHDAYS = 0x7FFFFFFF;
...
function monthDayMaskToStr(md){
  if(md===0) return 'None';
  if(md===ALL_MONTHDAYS || md===0xFFFFFFFF) return 'All month days';
  let s='';
...
function nightBadge(sc){
  ...
  const days = (sc.days || 0) & 0x7F;
  if(days === 0)
    return '<span class="night" style="background:#FFEBEE;color:#C62828">&#x26A0; No days selected</span>';
  const mm = ((sc.monthMask === undefined) ? 0x0FFF : (sc.monthMask & 0x0FFF));
  if(mm === 0)
    return '<span class="night" style="background:#FFEBEE;color:#C62828">&#x26A0; No months selected</span>';
  const a=sc.startHour*3600+sc.startMinute*60+sc.startSecond;
  const b=sc.stopHour*3600+sc.stopMinute*60+sc.stopSecond;
  const ds=dayMaskToStr(days);
  const ms=monthDayMaskToStr(sc.monthDays||0);
  const mmS=(mm===0x0FFF)?'All months':monthMaskToStr(mm);
  let info=ds;
  if(ms && ms!=='All month days') info+=' | Days:'+ms;
  if(mmS!=='All months') info+=' | Months:'+mmS;
...
function startEditName(relayIdx) {
  if(!relays[relayIdx]) { toast('Relay list changed - reloading', false); load(); return; }
  ...
  input.onblur = () => { if(editingRelay === relayIdx) saveNameEdit(relayIdx, input.value); };
  input.onkeydown = (e) => {
    if (e.key === 'Enter') {
      e.preventDefault();
...
function saveNameEdit(relayIdx, newName) {
  if(editingRelay !== relayIdx) return;   
  editingRelay = -1;                      
  const capturedInput = editingInput;
  editingInput = null;
  ...
  if(relays[relayIdx]) relays[relayIdx].name = newName;
  if (nameSpan) {
    nameSpan.textContent = newName;
    nameSpan.style.display = '';
  }
  if (capturedInput) capturedInput.remove();
  ...
  .then(r => r.json())
  .then(d => {
    if (d.success) toast('Name saved!');
    else toast('Failed to save name', false);
  })
...
<div class="slist">`;
    for(let s=0;s<NS;s++){
      ...
      const dayBits = (sc2.days === undefined) ? 0x7F : sc2.days;
      const monthDayBits = (sc2.monthDays === undefined) ? ALL_MONTHDAYS : sc2.monthDays;
      const monthMask = (sc2.monthMask === undefined) ? 0x0FFF : (sc2.monthMask & 0x0FFF);
...
function toggleDay(ri,si,dayIdx){
  if(busy) return;
  if(!relays[ri] || !relays[ri].schedules[si]) return;
  const mask = 1<<dayIdx;
  relays[ri].schedules[si].days ^= mask;
  const dayEl = document.getElementById('day_'+ri+'_'+si);
  if(dayEl && dayEl.children[dayIdx]) dayEl.children[dayIdx].className = 'day' + ((relays[ri].schedules[si].days & mask)?' on':'');
...
function toggleMonthDay(ri,si,dayIdx){
  ...
  if(!relays[ri] || !relays[ri].schedules[si]) return;
  const mask = 1<<dayIdx;
  let md = relays[ri].schedules[si].monthDays;
  if(md === undefined) md = ALL_MONTHDAYS;
  md ^= mask;
  relays[ri].schedules[si].monthDays = md;
  const mdayEl = document.getElementById('mday_'+ri+'_'+si);
  if(mdayEl && mdayEl.children[dayIdx]) mdayEl.children[dayIdx].className = 'mday' + ((md & mask)?' on':'');
  const nb=document.getElementById('nb_'+ri+'_'+si);
  if(nb)nb.innerHTML=nightBadge(relays[ri].schedules[si]);
}
...
function toggleMonth(ri,si,mIdx){
  ...
  mm = (mm & 0x0FFF) ^ mask;
  relays[ri].schedules[si].monthMask = mm;
  const monEl = document.getElementById('mon_'+ri+'_'+si);
  if(monEl && monEl.children[mIdx]) monEl.children[mIdx].className = 'month' + ((mm & mask)?' on':'');
  const nb=document.getElementById('nb_'+ri+'_'+si);
  if(nb)nb.innerHTML=nightBadge(relays[ri].schedules[si]);
}
...
function mc(ri,state){
  if(busy){ toast('Please wait for current save to finish', false); return; }
  if(!relays[ri]) return;
  if(editingRelay === ri && editingInput) saveNameEdit(ri, editingInput.value);
  else if(editingRelay !== -1) cancelEdit();
...
function save(ri){
  if(!relays[ri]){ toast('Relay not found', false); load(); return; }
  if(busy){ toast('Please wait...', false); return; }
  if(editingRelay === ri && editingInput) saveNameEdit(ri, editingInput.value);
  busy=true;
...
void updateScheduleCache() {
    ...
    for (int i = 0; i < gpioConfig.count; i++) {
        ...
        for (int s = 0; s < 8; s++) {
            ...
            uint16_t monthMask = relayConfigs[i].schedule.monthMask[s] & 0x0FFF;
            uint32_t monthDayMask = relayConfigs[i].schedule.monthDays[s] & MONTHDAY_ALL;            
            ...
            if (monthMask == 0) continue;
            if (monthDayMask == 0) continue;
            if (start == stop) {
                if (monthMask != MONTH_ALL && !(monthMask & (1 << currentMonth))) continue;
                if (monthDayMask != MONTHDAY_ALL && !(monthDayMask & (1 << (currentMonthDay - 1)))) continue;
                ...
                if (cur >= start) {
                    ...
                    if (checkDate) {
                        if (monthMask != MONTH_ALL && !(monthMask & (1 << checkDate->tm_mon))) continue;
                        if (monthDayMask != MONTHDAY_ALL && !(monthDayMask & (1 << (checkDate->tm_mday - 1)))) continue;
...
void processRelaySchedules() {
    ...
    for (int i = 0; i < gpioConfig.count; i++) {
        ...
        for (int s = 0; s < 8; s++) {
            ...
            if (monthDayMask == 0) continue;
            if (start == stop) {
                if (monthMask != MONTH_ALL && !(monthMask & (1 << currentMonthVal))) continue;
                if (monthDayMask != MONTHDAY_ALL && !(monthDayMask & (1 << (monthDay - 1)))) continue;
...
void loadConfiguration() {
    ...
    bool relayConfigsValid = (len == sizeof(RelayConfig) * MAX_RELAYS);
    preferences.end();
    if (!relayConfigsValid) {
        for (int i = 0; i < MAX_RELAYS; i++) {
            initScheduleDefaults(i);
        }
    } else if (sysConfig.version < 13) {
        for (int i = 0; i < MAX_RELAYS; i++) {
            for (int s = 0; s < 8; s++) {
                if (relayConfigs[i].schedule.monthMask[s] == 0) {
                    relayConfigs[i].schedule.monthMask[s] = MONTH_ALL;
                }
                if (relayConfigs[i].schedule.monthDays[s] == 0) {
                    relayConfigs[i].schedule.monthDays[s] = MONTHDAY_ALL;
                }
            }
        }
    }
    if (sysConfig.version < 13) {
        sysConfig.version = 13;
        saveConfiguration();
    }
...
void handleSaveRelay() {
    ...
    for (JsonObject sch : schedules) {
        ...
        relayConfigs[relay].schedule.days[s]        = (sch["days"] | 0) & 0x7F;
        relayConfigs[relay].schedule.monthDays[s]   = (sch["monthDays"] | 0) & MONTHDAY_ALL;
        relayConfigs[relay].schedule.monthMask[s]   = (sch["monthMask"] | 0) & 0x0FFF;   
        s++;
    }    
    saveConfiguration();
    updateScheduleCache();
...
void handleRelayName() {
    ...
    if (relay >= 0 && relay < gpioConfig.count) {
        if (name && strlen(name) > 0) {
            ...
            snprintf(relayConfigs[relay].name, 16, "Relay %d", relay + 1);
        }
        saveConfiguration();
        responseCache.valid = false;
        server.send(200, "application/json", "{\"success\":true}");
