...
void loop() {
    ...
    if (rtcPresent && rtcTimeValid) {
        if (timeHasElapsed(now, lastRTCDSync, DS3231_SYNC_INTERVAL)) {
            lastRTCDSync = now;
            DateTime ds3231Now = rtc.now();
            if (ds3231Now.year() >= 2024 && ds3231Now.year() <= 2100) {
                time_t ds3231Epoch = ds3231Now.unixtime();                
                if (VALID_UNIX_TIME(ds3231Epoch)) {
                    uint8_t previousSource = timeSource;
                    syncInternalRTC(ds3231Epoch);
                    if (previousSource != TIME_SOURCE_NTP) {
                        timeSource = TIME_SOURCE_RTC;
                    }
                } else {
                    rtcTimeValid = false;
