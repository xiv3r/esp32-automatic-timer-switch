...
// =============================================================================
//  Global Static Json Buffers
// =============================================================================
static StaticJsonDocument<2048> relayDoc;       
static StaticJsonDocument<2048> saveRelayDoc;   
static StaticJsonDocument<1024> systemDoc;      
static StaticJsonDocument<512>  gpioDoc;        
static StaticJsonDocument<4096> wifiScanDoc;    
...
void processNTPResponse() {
    ...
    if (packetSize >= NTP_PACKET_SIZE) {
        ...
        uint8_t version = (ntpPacketBuffer[0] >> 3) & 0x07;  
        if ((version >= 3) && mode == 4 && li != 3) {
    ...
    if (wifiConnecting) {
        ...
        if (status == WL_CONNECTED) {
            ...
            if (wifiReconnectAttempts >= MAX_RECONNECT && !wifiFirstAttempt) {
                ...
                wifiReconnectAttempts = MAX_RECONNECT + 1;  
            }
        }
    }
...
void handleSaveRelay() {
    ...
    saveRelayDoc.clear();      
    DeserializationError err = deserializeJson(saveRelayDoc, server.arg("plain"));
    if (err) {
        server.send(400, "application/json", "{\"success\":false,\"error\":\"Bad JSON\"}"); 
        return;
    }        
    int relay = saveRelayDoc["relay"];
    if (relay < 0 || relay >= gpioConfig.count) {
        server.send(400, "application/json", "{\"success\":false,\"error\":\"Invalid relay\"}");
        return;
    }        
    JsonArray schedules = saveRelayDoc["schedules"].as<JsonArray>();
    int s = 0;
...
void handleWiFiScanResults() {
    ...
    if (scanInProgress) {
        ...
        if (wifiPausedForScan) {
            ...
            if (millis() - lastScanAttempt > 30000) {
                wifiPausedForScan = false;
            }
        }
    }        
    ...
    JsonArray nets = wifiScanDoc.createNestedArray("networks");        
    ...
    scanResultCount = -1;        
    String resp; 
    serializeJson(wifiScanDoc, resp);
    server.send(200, "application/json", resp);
}
...
void handleGetSystem() {
    ...
    systemDoc["ap_ip"] = WiFi.softAPIP().toString();
    systemDoc["uptime"] = millis() / 1000UL;
    systemDoc["freeHeap"] = ESP.getFreeHeap();   
    char epochStr[32];
    snprintf(epochStr, sizeof(epochStr), "%llu", (unsigned long long)getCurrentEpoch());
    systemDoc["utcEpoch"] = epochStr;          
    String timeSourceStr = "None";
    if (timeSource == TIME_SOURCE_NTP) timeSourceStr = "NTP";
    else if (timeSource == TIME_SOURCE_BROWSER) timeSourceStr = "Browser";
    else if (timeSource == TIME_SOURCE_RTC) timeSourceStr = "RTC";
    systemDoc["timeSource"] = timeSourceStr;        
    systemDoc["ntpServer"] = sysConfig.ntp_server;
    systemDoc["ntpSyncAge"] = lastNTPSync > 0 ? (unsigned long)((millis() - lastNTPSync) / 1000UL) : (unsigned long)0xFFFFFFFF;  
    systemDoc["browserSyncAge"] = lastBrowserSync > 0 ? (unsigned long)((millis() - lastBrowserSync) / 1000UL) : (unsigned long)0xFFFFFFFF;  
    systemDoc["rtcSyncAge"] = lastRTCDSync > 0 ? (unsigned long)((millis() - lastRTCDSync) / 1000UL) : (unsigned long)0xFFFFFFFF; 
    systemDoc["wifiConnected"] = wifiConnected;
    systemDoc["wifiSSID"] = sysConfig.sta_ssid;
    systemDoc["rssi"] = wifiConnected ? (int)WiFi.RSSI() : 0;
    systemDoc["version"] = EEPROM_VERSION;
    systemDoc["chipModel"] = "ESP32-38P";
    systemDoc["mdnsHostname"] = getMDNSHostname();
    systemDoc["mdnsStarted"] = mdnsStarted;
    systemDoc["relayCount"] = gpioConfig.count;
    systemDoc["maxRelays"] = MAX_RELAYS;
    systemDoc["gmtOffset"] = sysConfig.gmt_offset;
    systemDoc["driftComp"] = driftCompensation;
    systemDoc["globalActiveMode"] = extConfig.global_active_mode;
    systemDoc["rtcPresent"] = rtcPresent;
    systemDoc["staEnabled"] = extConfig.sta_enabled ? true : false;      
    String resp;
    serializeJson(systemDoc, resp);
    server.send(200, "application/json", resp);
}
...
void handleGetGPIOConfig() {
    lastConnectionActivity = millis();    
    gpioDoc.clear();      
    gpioDoc["count"] = gpioConfig.count;
    gpioDoc["maxRelays"] = MAX_RELAYS;        
    JsonArray pins = gpioDoc.createNestedArray("pins");
    JsonArray activeLow = gpioDoc.createNestedArray("activeLow");
    for (int i = 0; i < gpioConfig.count; i++) {
        pins.add(gpioConfig.pins[i]);
        activeLow.add(gpioConfig.activeLow[i]);
    }        
    JsonArray available = gpioDoc.createNestedArray("availablePins");    
    // change gpio  
