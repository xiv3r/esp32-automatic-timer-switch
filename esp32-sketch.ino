...
void syncInternalRTCFromDS3231() {
    ...
    if (nowRTC.year() >= 2020 && nowRTC.year() <= 2100) {
        ...
        if (VALID_UNIX_TIME(rtcEpoch)) {
            time_t currentEpoch = (rtcInitialized && internalEpoch > 0) ? getCurrentEpoch() : 0;
            if (!rtcInitialized || internalEpoch == 0 || 
                abs((long)(rtcEpoch - currentEpoch)) > 1) {
                ...
                if (timeSource != TIME_SOURCE_NTP && timeSource != TIME_SOURCE_BROWSER) {
...
// =============================================================================
//  recoverRTC - DS3231 is authoritative, internal RTC is fallback
// =============================================================================
...
void loop() {
    ...
    checkAndCleanMemory();    
    if (rtcPresent && timeHasElapsed(now, lastRTCDSync, DS3231_SYNC_INTERVAL)) {
        lastRTCDSync = now;
