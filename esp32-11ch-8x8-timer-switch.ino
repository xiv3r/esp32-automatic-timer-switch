...
#define EEPROM_VERSION 2              // Version bumped to 2 for 8-schedule support
...
// ESP32 GPIO pin mapping (choose pins that are safe for output)
const int relayPins[NUM_RELAYS] = {
  32, 33, 25, 26, 27, 14, 12, 13, 15, 2, 4
};
...
<body>
    ...
    <script>
        ...
        let currentTime = '';
        let isLoading = false;
        const NUM_SCHEDULES = 8;   // Changed to 8 schedules
...
void setup() {
    ...
    // Initialize relay configs with defaults (8 schedules)
    for (int i = 0; i < NUM_RELAYS; i++) {
        for (int s = 0; s < 8; s++) {
    ...
    // Load configuration (handles migration)
    loadConfiguration();
    loadRTCState();
...
void processRelaySchedules() {
    ...
    for (int i = 0; i < NUM_RELAYS; i++) {
        ...
        for (int s = 0; s < 8; s++) {      // Check all 8 schedules
...
void loadConfiguration() {
    ...
    if (sysConfig.magic != EEPROM_MAGIC || sysConfig.version != EEPROM_VERSION) {
        if (sysConfig.magic == EEPROM_MAGIC && sysConfig.version == 1) {
            // Migrate from version 1 (4 schedules) to version 2 (8 schedules)
            Serial.println("Migrating EEPROM from version 1 to 2 (4->8 schedules)");
            
            // Old RelayConfig with 4 schedules
            ...
            for (int i = 0; i < NUM_RELAYS; i++) {
                ...
                // Initialize schedules 4-7 to disabled/default
                for (int s = 4; s < 8; s++) {
        ...
        // Normal load of version 2 config
        int addr = sizeof(SystemConfig);
        for (int i = 0; i < NUM_RELAYS; i++) {
...
void handleGetRelays() {
    DynamicJsonDocument doc(8192);   // Increased for 8 schedules
